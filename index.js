...
app.get("/studBucket5", (req, res) => {
  res.render("studBucket5", { name: req.user.name, email: req.user.username });
});
...
app.get("/profile/studentBuckets", (req, res) => {
  res.render("facultySubmittedStudentB", {
    name: req.user.name,
...
app.get("/profile/reportDownload", async (req, res) => {
  try {
    ...
    // Fetch appraisal data from MongoDB
    const userDocument = await studBucket.findOne({ name: req.user.username });
...
app.get("/profile/studentBuckets/forms1", async (req, res) => {
  try {
    ...
    if (userDocument && userDocument.sd1 && userDocument.sd1.length > 0) {
      res.render("facultySubmittedForms1", {
        user: req.user,
        name: req.user.name,
        email: req.user.username,
        sd1: userDocument.sd1,
        avgSelfAppraisalScore1: userDocument.avgSelfAppraisalScore1,
        reviewerScore1: userDocument.reviewerScore1,
        comments1: userDocument.comments1,
      });
    } else {
      res.render("facultySubmittedForms1", {
        user: req.user,
...
app.get("/profile/studentBuckets/forms2", async (req, res) => {
  try {
    ...
    if (userDocument && userDocument.sd2 && userDocument.sd2.length > 0) {
      res.render("facultySubmittedForms2", {
        user: req.user,
        name: req.user.name,
        email: req.user.username,
        sd2: userDocument.sd2,
        avgSelfAppraisalScore2: userDocument.avgSelfAppraisalScore2,
        reviewerScore2: userDocument.reviewerScore2,
        comments2: userDocument.comments2,
      });
    } else {
      res.render("facultySubmittedForms2", {
        user: req.user,
...
app.get("/profile/studentBuckets/forms3", async (req, res) => {
  try {
    ...
    if (userDocument && userDocument.sd3 && userDocument.sd3.length > 0) {
      res.render("facultySubmittedForms3", {
        user: req.user,
        name: req.user.name,
        email: req.user.username,
        sd3: userDocument.sd3,
        avgSelfAppraisalScore3: userDocument.avgSelfAppraisalScore3,
        reviewerScore3: userDocument.reviewerScore3,
        comments3: userDocument.comments3,
      });
    } else {
      res.render("facultySubmittedForms3", {
        user: req.user,
...
app.get("/profile/studentBuckets/forms4", async (req, res) => {
  try {
    ...
    if (userDocument && userDocument.sd4 && userDocument.sd4.length > 0) {
      res.render("facultySubmittedForms4", {
        user: req.user,
        name: req.user.name,
        email: req.user.username,
        sd4: userDocument.sd4,
        avgSelfAppraisalScore4: userDocument.avgSelfAppraisalScore4,
        reviewerScore4: userDocument.reviewerScore4,
        comments4: userDocument.comments4,
      });
    } else {
      res.render("facultySubmittedForms4", {
        user: req.user,
...
app.get("/profile/studentBuckets/forms5", async (req, res) => {
  try {
    ...
    if (userDocument && userDocument.sd5 && userDocument.sd5.length > 0) {
      res.render("facultySubmittedForms5", {
        user: req.user,
        name: req.user.name,
        email: req.user.username,
        sd5: userDocument.sd5,
        avgSelfAppraisalScore5: userDocument.avgSelfAppraisalScore5,
        reviewerScore5: userDocument.reviewerScore5,
        comments5: userDocument.comments5,
      });
    } else {
      res.render("facultySubmittedForms5", {
        user: req.user,
...
app.post(
  "/profile/studentBuckets/forms1/comment",
  checkRole("faculty"),
  async (req, res) => {
    try {
...
app.post(
  "/profile/studentBuckets/forms2/comment",
  checkRole("faculty"),
  async (req, res) => {
    try {
      const username = req.user.username;
...
app.post(
  "/profile/studentBuckets/forms3/comment",
  checkRole("faculty"),
  async (req, res) => {
    try {
      const username = req.user.username;
...
app.post(
  "/profile/studentBuckets/forms4/comment",
  checkRole("faculty"),
  async (req, res) => {
    try {
      const username = req.user.username;
...
app.post(
  "/profile/studentBuckets/forms5/comment",
  checkRole("faculty"),
  async (req, res) => {
    try {
      const username = req.user.username;
...
app.get("/approve/StudentBucket/:name", checkRole("ro"), async (req, res) => {
  try {
    ...
    if (user) {
      res.render("SubmittedstudentBuckets", {
        user,
        name: req.user.name,
        email: req.user.username,
      });
...
app.get(
  "/approve/submittedformsSD1/:name",
  checkRole("ro"),
  async (req, res) => {
    try {
      const { name } = req.params;
...
app.get(
  "/approve/submittedformsSD2/:name",
  checkRole("ro"),
  async (req, res) => {
    try {
      const { name } = req.params;
...
app.get(
  "/approve/submittedformsSD3/:name",
  checkRole("ro"),
  async (req, res) => {
    try {
      const { name } = req.params;
...
app.get(
  "/approve/submittedformsSD4/:name",
  checkRole("ro"),
  async (req, res) => {
    try {
      const { name } = req.params;
...
app.post(
  "/approve/submittedformsSD1/:name/comment",
  checkRole("ro"),
  async (req, res) => {
    try {
      const { name } = req.params;
...
app.post(
  "/approve/submittedformsSD2/:name/comment",
  checkRole("ro"),
  async (req, res) => {
    try {
      const { name } = req.params;
...
app.post(
  "/approve/submittedformsSD3/:name/comment",
  checkRole("ro"),
  async (req, res) => {
    try {
      const { name } = req.params;
...
app.post(
  "/approve/submittedFormsSD1/:name/save",
  checkRole("ro"),
  async (req, res) => {
    try {
      const { name } = req.params;
...
app.post(
  "/approve/submittedFormsSD2/:name/save",
  checkRole("ro"),
  async (req, res) => {
    try {
      const { name } = req.params;
...
app.post("/signin", function (req, res) {
  // Temporary User object is created
  const user = new User({
    username: req.body.username,
    password: req.body.password,
  });

  // You are telling Passport: “Try logging in this user”
  ...
  try {
    ...
    if (req.isAuthenticated() && req.user && req.user.username) {
      ...
      if (file) {
        ...
        // calculates the average score of all SD1 entries submitted by the user.
        if (userDocument.sd1.length > 0) {
          avgSubTotal1 =
            userDocument.sd1.reduce((total, sd) => total + sd.subTotal1, 0) /
            userDocument.sd1.length;
        }
        ...
        return res.send(
          "<script>alert('You data is saved Successfully'); window.location.href = '/studBucket1'; clearSelectTags();</script>"
        );
      } else {
    ...
    if (req.isAuthenticated() && req.user && req.user.username) {
      ...
      if (file) {
        ...
        if (userDocument.sd2.length > 0) {
          avgSubTotal2 =
            userDocument.sd2.reduce((total, sd) => total + sd.subTotal2, 0) /
            userDocument.sd2.length;
        }
        ...
        return res.send(
          "<script>alert('You data is saved Successfully'); window.location.href = '/studBucket2'; clearSelectTags();</script>"
        );
      } else {
        res.status(500).send("file not found");
      }
...
app.post("/save3", upload.single("file"), async (req, res) => {
    ...
    try {
      ...
      if (file) {
        ...
        if (userDocument.sd3.length > 0) {
          avgSubTotal3 =
            userDocument.sd3.reduce((total, sd) => total + sd.subTotal3, 0) /
            userDocument.sd3.length;
        }
        ...
        let totalScore = userDocument.totalSelfAppraisalScore || 0;

        // Add avgSelfAppraisalScore1 to total
        totalScore += userDocument.avgSelfAppraisalScore3 || 0;

        // Update totalSelfAppraisalScore in the document
        const updateTotalScoreResult = await studBucket.updateOne(
          { name: req.user.username },
          { $set: { totalSelfAppraisalScore: totalScore } }
        );
...
app.post("/save4", async (req, res) => {
  try {
    if (req.isAuthenticated()) {
      ...
      const userDocument = await studBucket.findOne({
        name: req.user.username,
      });
      ...
      res.send(
        "<script>alert('Your data is saved Successfully'); window.location.href = '/studBucket2'; clearSelectTags();</script>"
      );
    } else {
